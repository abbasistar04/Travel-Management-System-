...
const MOCK_SALARY_SHEET = [
  { name: 'Ahmad Ali', employeeId: '001', department: 'Engineering', status: 'Inactive' },
  { name: 'Sara Khan', employeeId: '002', department: 'Sales', status: 'Inactive' },
  { name: 'Usman Ahmed', employeeId: '003', department: 'Accountant', status: 'Inactive' },
  { name: 'Fatima Noor', employeeId: '004', department: 'Engineering', status: 'Inactive' },
]

export function EmployeeSalarySheetPage() {
  ...
  const openEditPanel = (row) => {
    setEditFormEmployeeName(row.name ?? '')
    setEditFormDepartment(row.department ?? '')
    setEditFormStatus(row.status ?? '')
    setEditFormDesignation('')
    setEditFormBasic('')
    setEditFormCommission('')
    setEditFormBonus('')
    setEditFormDate('')
  ...
  return (
    <div className="flex-1 overflow-auto flex flex-col max-h-[90vh] overflow-hidden min-h-0 bg-white relative">
      {/* Slide-over: Add Employee - Create/Edit Employee salary record */}
      {showAddPanel && (
        <>
      ...
      <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
        ...
        {/* Filter row: Search Employee, Department, Salary Status, Payment Method, Date */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <div className="rounded-lg border flex flex-col max-h-[90vh] overflow-hidden min-w-0 justify-center" style={{ borderColor: FILTER_BORDER, backgroundColor: FILTER_BG }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="rounded-lg border border-slate-200 p-4 flex flex-col max-h-[90vh] overflow-hidden min-w-0" style={{ backgroundColor: CARD_1 }}>
          ...
          <div className="rounded-lg border border-slate-200 p-4 flex flex-col max-h-[90vh] overflow-hidden min-w-0" style={{ backgroundColor: CARD_2 }}>
            <span className="text-xs font-medium text-slate-600">Active Employee</span>
            <span className="text-2xl font-bold text-slate-800 mt-1">110</span>
          </div>
          <div className="rounded-lg border border-slate-200 p-4 flex flex-col max-h-[90vh] overflow-hidden min-w-0" style={{ backgroundColor: CARD_3 }}>
            <span className="text-xs font-medium text-slate-600">Inactive Employee</span>
            <span className="text-2xl font-bold text-slate-800 mt-1">3</span>
          </div>
          <div className="rounded-lg border border-slate-200 p-4 flex flex-col max-h-[90vh] overflow-hidden min-w-0" style={{ backgroundColor: CARD_4 }}>
              ...
              <tbody>
                {MOCK_SALARY_SHEET.map((row, idx) => (
                  <tr key={idx} className="border-t border-slate-200 hover:bg-slate-50/50 bg-white">
                    <td className="py-3 px-4" style={{ color: '#000000' }}>{row.name}</td>
                    <td className="py-3 px-4" style={{ color: '#000000' }}>{row.employeeId}</td>
                    <td className="py-3 px-4" style={{ color: '#000000' }}>{row.department}</td>
                    <td className="py-3 px-4">
                      <span className="inline-flex items-center gap-1.5 whitespace-nowrap">
                        <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: STATUS_ORANGE }} aria-hidden />
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pt-4 pb-6">
          ...
          <button
            type="button"
            onClick={() => navigate('/dashboard/hrm-attendance')}
            className="inline-flex items-center justify-center font-semibold rounded-lg px-6 py-3 text-white hover:opacity-90 tracking-wide shadow-lg"
            style={{ backgroundColor: BRAND_RED }}
          >
            HRM attendance
