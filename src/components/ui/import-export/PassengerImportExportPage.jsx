...
const MOCK_IMPORT_LOG = [
  { fileName: 'Passenger_27 july.cvs', status: 'Success', passengerType: 'Adult/Child', uploadDate: '03/03/2023', uploadBy: 'Admin' },
  { fileName: 'Passenger_27 july.cvs', status: 'Success', passengerType: 'Adult/Child', uploadDate: '03/03/2023', uploadBy: 'Admin' },
  { fileName: 'Passenger_27 july.cvs', status: 'Success', passengerType: 'Adult/Child', uploadDate: '03/03/2023', uploadBy: 'Admin' },
  { fileName: 'Passenger_27 july.cvs', status: 'Success', passengerType: 'Adult/Child', uploadDate: '03/03/2023', uploadBy: 'Admin' },
]

export function PassengerImportExportPage() {
  ...
  return (
    <div className="flex-1 overflow-auto flex flex-col min-h-0 bg-white relative">
      ...
      <div className="p-4 sm:p-6 space-y-4 sm:space-y-6 min-w-0">
        {/* Header: title left, Add Passenger + Export All right */}
        ...
        {/* Main action buttons: Passenger Import ↓, Passenger Export ↑ */}
              ...
              <table className="w-full text-sm text-left border-collapse" style={{ minWidth: '700px' }}>
                <thead>
                  <tr className="font-semibold text-slate-700 bg-slate-50 border-b border-slate-200">
                    ...
                    <th className="text-left py-3 px-4">Passenger Type</th>
                ...
                <tbody>
                  {MOCK_IMPORT_LOG.map((row, idx) => (
                    <tr key={idx} className="border-b border-slate-100 hover:bg-slate-50/50 bg-white last:border-b-0">
                      ...
                      <td className="py-3 px-4 text-slate-900">{row.passengerType}</td>
                      <td className="py-3 px-4 text-slate-900">{row.uploadDate}</td>
                      <td className="py-3 px-4 text-slate-900">{row.uploadBy}</td>
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-2">
                          <button type="button" onClick={() => { setEditingRow({ ...row, date: row.uploadDate }); setShowPanel(true) }} className="p-1.5 rounded hover:opacity-80 transition" aria-label="Edit">
